CREATE PROCEDURE [dbo].[P_TaskScheduler_Upd]
(
    @id                     UNIQUEIDENTIFIER,
	@name                   NVARCHAR(50),
...
BEGIN
	...
	BEGIN
		 ...
		 (
		  ...
		  DELETE FROM [dbo].[ParameterSettingTask] WHERE TaskSchedulerId = @id
		...
		BEGIN
			SET @result = 0
			SET @errorMessage = 'Unknown error, failed to update ParameterSettingTask'
			GOTO P_Failed
		END
	END
	...
	BEGIN
		 ...
		 DELETE FROM [dbo].[InstrumentSettings] WHERE TaskSchedulerId = @id
		 INSERT INTO [dbo].[InstrumentSettings]
           ([Id]
           ,[TaskSchedulerId]
           ,[ExchangeCode]
           ,[InstrumentId]
		 ...
		 BEGIN
			SET @result = 0
			SET @errorMessage = 'Unknown error, failed to update  InstrumentSettings'
			GOTO P_Failed
		 END
	END
   ...
   WHERE Id = @id
	...
	BEGIN
        SET @result = 0
	    SET @errorMessage = 'Unknown error, failed to add update TaskScheduler'
		GOTO P_Failed
	END
