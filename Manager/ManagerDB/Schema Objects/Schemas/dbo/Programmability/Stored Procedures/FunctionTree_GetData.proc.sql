...
BEGIN
	...
	BEGIN
		...
		SELECT pt2.Id,pt2.Code,(CASE WHEN @language='CHT' THEN fd.NameCHT ELSE (CASE WHEN @language='CHS' THEN fd.NameCHS ELSE fd.NameENG END) END)
		FROM @AllPermission ap 
		...
		FROM @AllPermission ap 
			INNER JOIN dbo.PermissionTarget pt1 ON pt1.Id=ap.ParentId
			INNER JOIN dbo.PermissionTarget pt2 ON pt2.Id=pt1.ParentId
			INNER JOIN dbo.FunctionDescription fd ON fd.FunctionId=pt1.Id
		WHERE ap.[Status]=1
	END
	ELSE
	BEGIN
