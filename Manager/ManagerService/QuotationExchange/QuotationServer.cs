...
namespace ManagerService.QuotationExchange
{
    public class QuotationServer
    {
        ...
        public bool Flush()
        {
            if (this._Cache != null)
            {
                Manager.Common.Logger.TraceEvent(TraceEventType.Information,"QuotationServer\r\n{0}", "Begin QuotationFileCache.Flush" );
                try
                {
                ...
                {
                    Manager.Common.Logger.TraceEvent(TraceEventType.Information,"QuotationServer\r\n{0}", "End QuotationFileCache.Flush" );
                }
            }
            return true;
        }
        ...
        public bool SetQuotation(Token token, List<GeneralQuotation> quotations, out iExchange.Common.OriginQuotation[] originQs, out iExchange.Common.OverridedQuotation[] overridedQs)
        {
            ...
            try
            {
                ...
                {
                    Manager.Common.Logger.TraceEvent(TraceEventType.Warning, "QuotationServer SetQuotation: has Invalid Quotation:{0}", quotations);
                }
        ...
        public bool DiscardQuotation(Token token, Guid instrumentID, out iExchange.Common.OriginQuotation[] originQs, out iExchange.Common.OverridedQuotation[] overridedQs)
        {
            ...
            try
            {
                ...
                if (instrument.ScheduleID != null)
                {
                    ...
                    {
                        Manager.Common.Logger.TraceEvent(TraceEventType.Warning, "QuotationServer\r\nDiscardQuotation--Id: {0}, IsTrading:{1}, DealerID: {2}, ScheduleID: {3}",
                            instrument.ID, instrument.IsTrading, instrument.LastOfferDealerID, instrument.ScheduleID);
                    }
        ...
        private void SetTradingState(object sender, object Args)
        {
            ...
            Manager.Common.Logger.TraceEvent(TraceEventType.Information, "QuotationServer\r\nSetTradingState({0}) start", tradingTime);
            ...
            try
            {
                ...
                {
                    Manager.Common.Logger.TraceEvent(TraceEventType.Warning, "QuotationServer\r\nSetTradingState({0}) is triggered in yesterday", tradingTime);
                    return;
                }
            ...
            {
                Manager.Common.Logger.TraceEvent(TraceEventType.Error, "QuotationServer\r\nSetTradingState({0}) failed at\r\n{1}", tradingTime, exception);
            }
            finally
            {
                this.rwLock.ReleaseWriterLock();
        ...
        private void ClearSessionQuotationItems(bool isDayOpenTime, Guid instrumentId)
        {
            ...
            {
                Manager.Common.Logger.TraceEvent(TraceEventType.Error, "QuotationServer\r\nClearSessionQuotationItems,cann't find Instrument for instrumentId:{0}", instrumentId);
                return;
            }
            ...
            {
                Manager.Common.Logger.TraceEvent(TraceEventType.Warning, "QuotationServer\r\nClearSessionQuotationItems,cann't find OriginQuotation for instrumentId:{0}", instrumentId);
            }
            ...
            foreach (QuotePolicyDetail quotePolicy in instrument.QuotePolicyDetails)
            {
                ...
                {
                    Manager.Common.Logger.TraceEvent(TraceEventType.Warning, "QuotationServer\r\nClearSessionQuotationItems,cann't find OverridedQuotation for instrumentId:{0}; quotePolicy.ID:{1}", instrumentId, quotePolicy.ID);
                }
            }
        }
        ...
        private bool ProcessSessionLastQuotations(Guid instrumentId, DateTime beginTime)
        {
            ...
            {
                Manager.Common.Logger.TraceEvent(TraceEventType.Error, "QuotationServer\r\nProcessSessionLastQuotations failed.{0}", exception.ToString());
                throw;
            }
        }
        ...
        private void QuotationWaitTimeout(object sender, object Args)
        {
            ...
            try
            {
                ...
                if (instrument.OriginCode == this.traceOriginCode)
                {
                    Manager.Common.Logger.TraceEvent(TraceEventType.Warning, "QuotationServer\r\nQuotationWaitTimeout: Id=={0}, IsTrading=={1}, DealerID=={2}, ScheduleID=={3}, bseTime=={4:yyyy-MM-dd HH:mm:ss.fff}, originQ=={5}, LastOrigin=={6}",
                        instrument.ID, instrument.IsTrading, instrument.LastOfferDealerID, instrument.ScheduleID, baseTime, originQ, instrument.LastOrigin);
                }
                instrument.ScheduleID = null;
                ...
                {
                    Manager.Common.Logger.TraceEvent(TraceEventType.Warning, "QuotationServer\r\nQuotationWaitTimeout--originQ.Bid > originQ.Ask [originQ:{0},LastOrigin:{1}]", originQ, instrument.LastOrigin);
                }
        ...
        private void OnTradeDayClose(object sender, object Args)
        {
            ...
            try
            {
                Manager.Common.Logger.TraceEvent(TraceEventType.Information, "QuotationnServerOnTradeDayClose: WaitTillReset {0}", tradeDay.Day);
                iExchange.Common.DataAccess.WaitTillReset(tradeDay.Day, this.connectionString);

                DateTime nextDay = tradeDay.Day.AddDays(1);
                Manager.Common.Logger.TraceEvent(TraceEventType.Information, "QuotationnServer\r\nOnTradeDayClose: TradeDay {0:yyyy-MM-dd}) Initialize Begin", nextDay);
                this.Initialize(this.GetData( this.connectionString, nextDay));
                Manager.Common.Logger.TraceEvent(TraceEventType.Information, "QuotationnServer\r\nOnTradeDayClose: TradeDay {0:yyyy-MM-dd} Initialize End", nextDay);
            }
            catch (Exception e)
            {
                Manager.Common.Logger.TraceEvent(TraceEventType.Error, "QuotationnServer{0}", e.ToString());
            }
            finally
            {
                this.rwLock.ReleaseWriterLock();
        ...
        private bool SetQuotation(Token token, CollectorQuotation[] quotations)
        {
            ...
            foreach (CollectorQuotation collectorQ in quotations)
            {
                ...
                foreach (Instrument instrument in instruments)
                {
                    if (instrument.OriginCode == this.traceOriginCode)
                    {
                        Manager.Common.Logger.TraceEvent(TraceEventType.Warning, "QuotationServer\r\nSetQuotation--Id: {0}, IsTrading:{1}, DealerID: {2}, ScheduleID: {3}",
                            instrument.ID, instrument.IsTrading, instrument.LastOfferDealerID, instrument.ScheduleID);
                    }
        ...
        private void UpdateOpenPrice()
        {
            ...
            {
                Manager.Common.Logger.TraceEvent(TraceEventType.Error, "QuotationServer\r\nUpdateOpenPrice failed:{0}", exception);
            }
        ...
        private bool SetQuotation(Token token, DealerQuotation[] quotations)
        {
            foreach (DealerQuotation dealerQ in quotations)
            {
                ...
                {
                    Manager.Common.Logger.TraceEvent(TraceEventType.Warning, "QuotationServer.SetQuotation\r\nQuotation: {0} is ingored for the instrument is not in trading", dealerQ);
                    continue;
                }
                ...
                if (instrument.ScheduleID != null)
                {
                    if (this.scheduler[instrument.ScheduleID] != null)
                    {
                        ...
                        if (((TimeSpan)(dealerQ.Timestamp - originQ.Timestamp)).TotalSeconds < 1)
                        {
                            if (instrument.OriginCode == this.traceOriginCode)
                            {
                                Manager.Common.Logger.TraceEvent(TraceEventType.Warning, "QuotationServer\r\nSetQuotation Dealer Right--Id: {0}, IsTrading:{1}, DealerID: {2}, ScheduleID: {3}",
                                    instrument.ID, instrument.IsTrading, instrument.LastOfferDealerID, instrument.ScheduleID);
                            }
                    ...
                    else
                    {
                        if (instrument.OriginCode == this.traceOriginCode)
                        {
                            Manager.Common.Logger.TraceEvent(TraceEventType.Warning, "QuotationServer\r\nSetQuotation Dealer Error--Id: {0}, IsTrading:{1}, DealerID: {2}, ScheduleID: {3}",
                                instrument.ID, instrument.IsTrading, instrument.LastOfferDealerID, instrument.ScheduleID);
                        }
        ...
        private void ProcessInstrument(OriginQuotation originQ, DateTime baseTime)
        {
            ...
            if (originQ.Origin != null)
            {
                ...
                else
                {
                    ...
                    if (instrument.OriginCode == this.traceOriginCode)
                    {
                        Manager.Common.Logger.TraceEvent(TraceEventType.Warning, "QuotationServer\r\nProcessInstrument: Id=={0}, IsTrading=={1}, DealerID=={2}, ScheduleID=={3}, WaitTime=={4}, baseTime=={5:yyyy-MM-dd HH:mm:ss.fff}, originQ=={6}, LastOrigin=={7}",
                            instrument.ID, instrument.IsTrading, instrument.LastOfferDealerID, instrument.ScheduleID, waitTime, baseTime, originQ, instrument.LastOrigin);
                    }
                }
            }
        ...
        private void ProcessQuotePolicy(DealerQuotation dealerQ)
        {
            ...
            {
                Manager.Common.Logger.TraceEvent(TraceEventType.Warning, "QuotationServer\r\nDealer change High, low: dealerQ=={0}", dealerQ);
            }
            ...
            where VType : PersistObject
        {
            using (SqlConnection connection = new SqlConnection(this.connectionString))
            {
                ...
                {
                    Manager.Common.Logger.TraceEvent(TraceEventType.Error,"QuotationServer{0}", exception.ToString());
        ...
        {
            Manager.Common.Logger.TraceEvent(TraceEventType.Information, "QuotationServer\r\nQuotationServer.Update(Token token, XmlNode update) \r\n{0}", update.OuterXml);
                    ...
                    foreach (XmlNode row in method.ChildNodes)
                    {
                        switch (row.Name)
                        {
                            ...
                            case "QuotePolicyDetail":
                                ...
                                {
                                    Manager.Common.Logger.TraceEvent(TraceEventType.Warning, "QuotationServer\r\nQuotationServer.Update QuotePolicyDetail--> not found: {0},{1}\r\n{2}", method.Name, row.Name, update.OuterXml);
                                }
                                break;
                            case "QuotePolicyDetails":
                                foreach (XmlNode row2 in row.ChildNodes)
                                {
                                    ...
                                    {
                                        Manager.Common.Logger.TraceEvent(TraceEventType.Warning, "QuotationServer\r\nQuotationServer.Update QuotePolicyDetails--> not found: {0},{1}\r\n{2}", method.Name, row2.OuterXml, update.OuterXml);
                                    }
                                }
                                break;
                            case "SystemParameter":
                                ...
                                break;
                            default:
                                Manager.Common.Logger.TraceEvent(TraceEventType.Warning, "QuotationServer\r\nQuotationServer.Update --> not found: {0},{1}\r\n{2}", method.Name, row.Name, update.OuterXml);
                                break;
            ...
            {
                Manager.Common.Logger.TraceEvent(TraceEventType.Error, "QuotationServer{0}", e.ToString());
            }
            finally
            {
                this.rwLock.ReleaseWriterLock();
        ...
        public void UpdateHighLow(Token token, string ip, Guid instrumentId, bool isOriginHiLo, string newInput, bool isUpdateHigh, out int batchProcessId, out string instrumentCode, out bool highBid, out bool lowBid, out DateTime updateTime, out DateTime minTimestamp, out iExchange.Common.OverridedQuotation[] overridedQs, out int returnValue, out string errorMessage)
        {
            ...
            if (!isOriginHiLo)
            {
                ...
                Manager.Common.Logger.TraceEvent(TraceEventType.Information,"QuotationServer.UpdateHighLow.Begin{0}", message);
                    ...
                    using (TransactionScope transactionScope = new TransactionScope(TransactionScopeOption.Required, transactionOption))
                    {
                        try
                        {
                            ...
                            if (returnValue != 0)
                            {
                                ...
                                Manager.Common.Logger.TraceEvent(TraceEventType.Warning, "QuotationServer.UpdateHighLow.EXEC P_UpdateHighLow(Failed){0}", message + "\r\n" + string.Format("Return {0}", returnValue));
                            }
                            else
                            {
                                ...
                                Manager.Common.Logger.TraceEvent(TraceEventType.Information, "QuotationServer.UpdateHighLow.EXEC P_UpdateHighLow(Succeed){0}", message + "\r\n" + string.Format("Return {0}", returnValue));
                            }
                        ...
                        catch (Exception exception)
                        {
                            ...
                            Manager.Common.Logger.TraceEvent(TraceEventType.Error, "QuotationServer.UpdateHighLow(Exception){0}", message + "\r\n" + string.Format("Failed:{0}", errorMessage));
                            return;
                        }
                        finally
                        {
                            ...
                            Manager.Common.Logger.TraceEvent(TraceEventType.Information, "QuotationServer.UpdateHighLow.End{0}", message);
                        }
                    }
                }
                finally
        ...
        public void RestoreHighLow(Token token, string ip, int batchProcessId, out Guid instrumentId, out string instrumentCode, out string newInput, out bool isUpdateHigh, out bool highBid, out bool lowBid, out DateTime minTimestamp, out iExchange.Common.OverridedQuotation[] overridedQs, out int returnValue, out string errorMessage)
        {
            ...
            try
            {
                ...
                Manager.Common.Logger.TraceEvent(TraceEventType.Information, "QuotationServer.RestoreHighLow.Begin{0}", message);
                this.Flush();
                using (TransactionScope transactionScope = new TransactionScope(TransactionScopeOption.Required, transactionOption))
                {
                    try
                    {
                        ...
                        if (returnValue != 0)
                        {
                            ...
                            Manager.Common.Logger.TraceEvent(TraceEventType.Warning, "QuotationServer.RestoreHighLow.EXEC P_RestoreHighLow(Failed){0}", message + "\r\n" + string.Format("Return {0}", returnValue));
                        }
                        else
                        {
                            ...
                            Manager.Common.Logger.TraceEvent(TraceEventType.Information, "QuotationServer.RestoreHighLow.EXEC P_RestoreHighLow(Succeed){0}", message + "\r\n" + string.Format("Return {0}", returnValue));
                        }
                        return;
                    }
                    catch (Exception exception)
                    {
                        returnValue = -10;
                        errorMessage += " [Exception:" + exception.ToString() + "]";
                        Manager.Common.Logger.TraceEvent(TraceEventType.Error, "QuotationServer.RestoreHighLow(Exception){0}", message + "\r\n" + string.Format("Failed:{0}", errorMessage));
                        return;
                    }
                    finally
                    {
                        Manager.Common.Logger.TraceEvent(TraceEventType.Information, "QuotationServer.RestoreHighLow.End{0}", message);
                    }
                }
            }
            finally
        ...
        public bool FixOverridedQuotationHistory(Token token, string quotation, bool needApplyAutoAdjustPoints, out iExchange.Common.OriginQuotation[] originQs, out iExchange.Common.OverridedQuotation[] overridedQs, out bool needBroadcastQuotation, out XmlNode fixChartDatas)
        {
            ...
            try
            {
                ...
                using (TransactionScope transactionScope = new TransactionScope(TransactionScopeOption.Required, transactionOption))
                {
                    ...
                    {
                        Manager.Common.Logger.TraceEvent(TraceEventType.Error, "QuotationServer\r\nSetHistoryQuotation failed:{0},{1}", quotation, exception);
                        return false;
                    }
                }
            }
        ...
        public bool UpdateOverridedQuotationHighLow(Token token, Guid instrumentID, string quotation, out iExchange.Common.OverridedQuotation[] overridedQs)
        {
            ...
            if (token.AppType == AppType.DealingConsole)
            {
                ...
                {
                    Manager.Common.Logger.TraceEvent(TraceEventType.Error, "QuotationServer\r\nUpdateOverridedQuotationHighLow failed:{0}", exception);
                    return false;
                }
            }
            return true;
