...
namespace ManagerService.DataAccess
{
    public class QuotationData
    {
        ...
        public static List<Manager.Common.ExchangeEntities.ChartQuotation> GetChartQuotation(string exchangeCode, Guid quotePolicyId, Guid instrumentId, string frequency, DateTime fromTime, DateTime toTime)
        {
            ...
            using (SqlConnection sqlConnection = DataAccess.GetInstance(exchangeCode).GetSqlConnection())
            {
                ...
                {
                    command.CommandType = CommandType.StoredProcedure;
                    command.CommandText = "dbo.P_GetChartData2";
                    command.Parameters.Add(new SqlParameter("@instrumentId", instrumentId));
                    command.Parameters.Add(new SqlParameter("@quotePolicyID", quotePolicyId));
                    command.Parameters.Add(new SqlParameter("@dataCycle", frequency));
                    command.Parameters.Add(new SqlParameter("@from", fromTime));
                    command.Parameters.Add(new SqlParameter("@to", toTime));

                    using (SqlDataReader reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            Manager.Common.ExchangeEntities.ChartQuotation chartQuotation = new Manager.Common.ExchangeEntities.ChartQuotation();
                            chartQuotation.Timestamp = (DateTime)reader["Date"];
                            chartQuotation.Open = double.Parse(reader["Open"].ToString());
                            chartQuotation.Close = double.Parse(reader["Close"].ToString());
                            chartQuotation.High = double.Parse(reader["High"].ToString());
                            chartQuotation.Low = double.Parse(reader["Low"].ToString());
                            chartQuotation.Volume = double.Parse(reader["Volume"].ToString());
                            chartQuotations.Add(chartQuotation);
                        }
                    }
                }
            }
        ...
        public static Manager.Common.ExchangeEntities.ChartQuotation GetLastQuotationsForTrendSheet(string exchangeCode, Guid quotePolicyId, Guid instrumentId, string frequency, DateTime fromTime, decimal open)
        {
            ...
            using (SqlConnection sqlConnection = DataAccess.GetInstance(exchangeCode).GetSqlConnection())
            {
                using (SqlCommand command = sqlConnection.CreateCommand())
                {
                    command.CommandText = "dbo.P_GetLastCloseForTrendSheet";
                    command.CommandType = CommandType.StoredProcedure;
                    command.Parameters.Add(new SqlParameter("@instrumentId", instrumentId));
                    command.Parameters.Add(new SqlParameter("@quotePolicyID", quotePolicyId));
                    command.Parameters.Add(new SqlParameter("@dataCycle", frequency));
                    command.Parameters.Add(new SqlParameter("@from", fromTime));
                    command.Parameters.Add(new SqlParameter("@open", open));
                    using (SqlDataReader reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            quotation.Timestamp = (DateTime)reader["Date"];
                            quotation.Close = double.Parse(reader["Close"].ToString());
                        }
                    }
                }
            }
