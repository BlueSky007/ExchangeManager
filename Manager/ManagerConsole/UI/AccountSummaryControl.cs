    ...
    public class TradingSummaryControl : Control
    {
       public delegate void GridExpandChangeHandler(bool isExpand, int rowIndex, int childCount);
        ...
        public override void OnApplyTemplate()
        {
            ...
            this._TradingSummaryGrid.CellClicked += new EventHandler<CellClickedEventArgs>(this._TradingSummaryGrid_CellClicked);
        }
        ...
        public void ExpandSubItemRow(bool isExpand, int rowIndex,int childCount)
        {
            ...
            if (subItemsCount == 0)
            {
                if (isExpand)
                {
                    this._TradingSummaryGrid.Rows[rowIndex].Height = new RowHeight(25 * (childCount + 1));
                }
                else
                {
